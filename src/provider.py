...
def GetSteamInstallPath() -> Path | None:
...
def GetSteamInstallPathWin32() -> Path | None:
    try:
        key = OpenKeyEx(HKEY_LOCAL_MACHINE, REG_STEAM)
...
def GetInstallationDirs(steam_installpath: Path) -> list[Path] | None:
...
def _ProcesACF(filepath: Path) -> tuple[int, str, Path] | None:
...
def BuildInstallationDictionary(gameinstalldirs: list[Path], only_appid = -1) -> dict[int, tuple[str, Path]]: 
...
def GetAppsDictionary() -> dict[int, tuple[str, Path]]:
...
def GetApp(app_id: int) -> tuple[str, Path] | None:
